...
SONG_COMPOSER_KEYS        = ["クリエイター", "作曲者", "Composer", "creator"]
...
POKE_TARGET_CAST_REL_KEYS   = ["target_cast_id", "対象者", "対象者参加者"]
POKE_CONCERT_REL_KEYS       = ["concert_id", "演奏会", "concert"]
POKE_PRACTICE_REL_KEYS      = ["practice_id", "練習", "practice"]
POKE_TYPE_KEYS              = ["poke_type", "種別", "type"]
POKE_MESSAGE_KEYS           = ["message", "本文", "メッセージ"]
POKE_STATUS_KEYS            = ["status", "ステータス"]
POKE_EXPIRES_AT_KEYS        = ["expires_at", "期限", "expires"]
POKE_CREATED_AT_KEYS        = ["created_at", "作成日時", "created_time"]
